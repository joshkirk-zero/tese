...
function render($app, $path, $data = array()){
    ...
    if(isset($WPGLOBAL["type"]) && $WPGLOBAL["type"] == "page" && !file_exists($file_path)){
        $file_path = views_dir() . "/page.php";
    }

    if(isset($WPGLOBAL["type"]) && $WPGLOBAL["type"] == "single" && !file_exists($file_path)){
